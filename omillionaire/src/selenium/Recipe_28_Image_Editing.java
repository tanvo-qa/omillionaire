...
@Test
public class Recipe_28_Image_Editing {
	...
	public void TC_03_Check_Sketch_To_Image() throws InterruptedException, AWTException {
		...
		driver.findElement(By.xpath("//p[1][contains(text(),'Image Editing')]")).click(); // Mở Image Editing recipe
																							// page
		...
		driver.findElement(
				By.xpath("//div[@class='max-w-full cursor-pointer']//p[@class='px-2' and text()='Remove Object']"))
		...
		WebElement successMsg_RemoveObject = wait.until(ExpectedConditions
				.visibilityOfElementLocated(By.xpath("//p[contains(text(),'Recipe has finished running!')]")));
		Assert.assertTrue(successMsg_RemoveObject.getText().contains("Recipe has finished running!"));
